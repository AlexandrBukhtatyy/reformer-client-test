...
export const insuranceApplicationValidation: ValidationSchemaFn<
...
> = (path: FieldPath<InsuranceApplicationForm>) => {
  ...
  number(path.coverageAmount, {
    message: "Страховая сумма должна быть числом",
  });
  min(path.coverageAmount, 10000, {
    message: "Минимальная страховая сумма 10 000 ₽",
  });
  ...
  applyWhen(
    ...
    (p) => {
      required(p.installments, {
        message: "Количество платежей обязательно при рассрочке",
      });
  ...
  applyWhen(
    ...
    (p) => {
      ...
      required(p.personalData.birthDate, {
      ...
      required(p.passportData.issueDate, {
      ...
      required(p.passportData.issuedBy, {
        message: "Кем выдан паспорт обязательно",
      });
    }
  );
  ...
  applyWhen(
    ...
    (p) => {
      ...
      required(p.companyData.ceoName, {
        message: "ФИО руководителя обязателен",
      });
    }
  );
  ...
  applyWhen(
    ...
    (p) => {
      ...
      max(p.vehicle!.year, new Date().getFullYear(), {
      ...
      min(p.vehicle!.mileage, 0, {
        message: "Пробег не может быть отрицательным",
      });
      required(p.vehicle!.enginePower, {
        message: "Мощность двигателя обязательна",
      });
      number(p.vehicle!.enginePower, {
        message: "Мощность должна быть числом",
      });
      min(p.vehicle!.enginePower, 1, {
        message: "Мощность должна быть больше 0",
      });
      required(p.vehicle!.licensePlate, { message: "Госномер обязателен" });
      required(p.vehicle!.registrationCert, {
        message: "Свидетельство о регистрации обязательно",
      });
    }
  );
  ...
  applyWhen(
    ...
    (p) => {
      ...
      max(p.property!.yearBuilt, new Date().getFullYear(), {
        message: `Максимальный год ${new Date().getFullYear()}`,
      });
      required(p.property!.wallMaterial, {
        message: "Материал стен обязателен",
      });
      required(p.property!.marketValue, {
        message: "Рыночная стоимость обязательна",
      });
      number(p.property!.marketValue, {
        message: "Стоимость должна быть числом",
      });
      min(p.property!.marketValue, 0, {
        message: "Стоимость не может быть отрицательной",
      });
      required(p.property!.ownershipDoc, {
        message: "Документ о собственности обязателен",
      });
    }
  );
  ...
  applyWhen(
    ...
    (p) => {
      ...
      applyWhen(
        ...
        (pp) => {
          required(pp.smokingYears, {
            message: "Стаж курения обязателен для курящих",
          });
          number(pp.smokingYears, {
            message: "Стаж должен быть числом",
          });
          min(pp.smokingYears, 0, {
      ...
      applyWhen(
        ...
        (pp) => {
          required(pp.chronicDiseases, {
            message: "Описание заболеваний обязательно",
          });
        }
      );
      ...
      applyWhen(
        ...
        (pp) => {
          required(pp.surgeries, {
            message: "Описание операций обязательно",
          });
        }
      );
    }
  ...
  applyWhen(
    ...
    (type) => type === "travel",
    (p) => {
      required(p.travel!.destination, {
        message: "Страна назначения обязательна",
      });
      required(p.travel!.tripPurpose, { message: "Цель поездки обязательна" });
      required(p.travel!.departureDate, {
        message: "Дата отъезда обязательна",
      });
      date(p.travel!.departureDate, { message: "Некорректная дата отъезда" });
      required(p.travel!.returnDate, {
        message: "Дата возвращения обязательна",
      });
  ...
  applyWhen(
    ...
    (p) => {
      ...
      required(p.previousPolicyEndDate, {
        message: "Дата окончания предыдущего полиса обязательна",
      });
  ...
  required(path.agreeTerms, {
    message: "Необходимо согласие с условиями страхования",
  });
  required(path.agreeElectronicPolicy, {
    message: "Необходимо согласие на электронный полис",
  });
  required(path.confirmAccuracy, {
    message: "Необходимо подтвердить достоверность информации",
  });
  required(path.electronicSignature, {
    message: "Электронная подпись обязательна",
  });

  // Policy data validation (these are computed/generated fields, so mostly format validation)
  // policyNumber is generated automatically
  // policyStartDate and policyEndDate are computed from other fields
};
